...
const TimeEdit = () => {
  ...
  const timeView = location.state.timeView;
  const user_name = location.state.user_name;



  const [timeData, setTimeData] = useState({
    start_time: timeView?.start_time || "09:00",
    end_time: timeView?.end_time || "18:00",
    rest_start_time: timeView?.rest_start_time || "12:00",
    rest_end_time: timeView?.rest_end_time || "13:00",
    time_id: timeView?.time_id
  });
  ...
  return (
    <div className="w-full h-full bg-gray-100">
      <div className="p-5 flex flex-1 flex-col gap-4 h-full">
        <form id="time-edit-form" onSubmit={handleSubmit} className="flex gap-4 flex-1 h-full">
          <div className="bg-white p-6 w-1/3 h-full flex flex-col border border-gray-200">
            <div className="mb-6">
              ...
              <p className="text-xs text-gray-500">시간을 수정할 직원 정보</p>
            </div>
